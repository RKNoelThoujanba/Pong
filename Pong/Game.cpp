...
namespace Pong
{
    Game::Game(const std::vector<const char*>& args)
    {
        ...
        {
            windowProperties.Width = windowProperties.Height = 0;
        }
        else
        {
        ...
        if(err != HGL::WindowError::NONE)
        {
            ...
            switch(err)
            {
            case HGL::WindowError::GLFW_INIT_FAILED:
                msg = "GLFW Init Failed\n";
                break;
            case HGL::WindowError::GLAD_LOAD_GL_FAILED:
                msg = "Glad Init Failed\n";
                break;
            case HGL::WindowError::WINDOW_CREATION_FAILED:
                msg = "GLFW Window Creation Failed\n";
                break;
            default:
                msg = "Unknown Window Error\n";
            }
